...
from torch.nn import BCELoss, CrossEntropyLoss, Dropout, Softmax, Linear, Conv2d, LayerNorm, Sigmoid
...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # attention_probs = self.smoid(attention_scores)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        # part_logits = self.smoid(part_logits)
        # result = (part_logits>self.threshold).float()*1
        # print(part_logits)
        if labels is not None:
            # print(labels.shape, part_logits.shape)
            ...
            # part_loss = loss_fct(part_logits.view(-1, self.num_classes).float(), labels.view(-1, self.num_classes).float())
