...
class dogs(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # smoid_label = np.zeros((120))
        # smoid_label[target_class] = 1
        # target_class = smoid_label ##BCE
        # print(self.classes[target_class])
        # if self.classes[target_class] == 'Chow':
        #     print(image_name)
...
class oxford(Dataset):
    ...
    def __init__(self,
        ...
        if self.train:
            self.annot_dir = join(self.root, 'annotations', 'trainval.txt')
        else:
            self.annot_dir = join(self.root, 'annotations', 'test.txt')
        
        with open(self.annot_dir, 'r') as f:
            for line in f:
                self._flat_breed_images.append((line.split(' ')[0], line.split(' ')[1]))
    ...
    def __getitem__(self, index):
        ...
        """
        image_name, target_class = self._flat_breed_images[index]
        target_class = int(target_class) -1
        image_path = join(self.images_folder, image_name+'.jpg')
        image = Image.open(image_path).convert('RGB')
